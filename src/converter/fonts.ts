...
import type { FontDescription, PdfPageProxy, PdfTextStyle } from "./types.ts";
...
export function describeFont(rawName: string, cssHint?: string): FontDescription {
  ...
  const style: FontDescription["style"] = ITALIC.test(name) ? "italic" : "normal";
 ...
 */
export function resolvePageFonts(
  page: PdfPageProxy,
  styles: Record<string, PdfTextStyle> | undefined,
): Map<string, FontDescription> {
  const out = new Map<string, FontDescription>();

  for (const [fontName, style] of Object.entries(styles ?? {})) {
    let real: { name?: string } | null = null;
    try {
