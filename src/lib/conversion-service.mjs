...
import { convert, ConversionError } from "../converter/index.mjs";
import { loadPdfjs, openPdf, rasterizePage, PdfOpenError } from "./pdf-client.mjs";

/** @typedef {"idle"|"reading"|"processing"|"done"|"failed"|"cancelled"} JobStatus */
...
export function createJob({ onChange }) {
  let state = { status: "idle", progress: 0, message: "", result: null, error: null };
  let cancelled = false;
  let runId = 0;
    ...
    async start(file, options) {
      ...
      try {
        ...
        const code = err instanceof PdfOpenError || err instanceof ConversionError
          ? err.code
          : /out of memory|allocation/i.test(err?.message ?? "") ? "OUT_OF_MEMORY" : "UNKNOWN";

        set({
          status: "failed",
          error: { code, message: FRIENDLY[code] ?? err?.message ?? FRIENDLY.UNKNOWN },
        });
      }
    },
...
export function buildZip(result, baseName) {
  const tree = {};
