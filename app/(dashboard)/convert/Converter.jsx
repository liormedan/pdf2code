...
import { createJob, buildZip, downloadBlob } from "@/src/lib/conversion-service.mjs";
import { loadPdfjs, openPdf, PdfOpenError } from "@/src/lib/pdf-client.mjs";
import { validateFile, MAX_PAGES, MAX_BYTES } from "@/src/lib/pricing.mjs";
import { useActivity } from "@/src/lib/session-activity";
import OutputPreview from "./OutputPreview";

const FORMATS = ["html", "react"];
...
export default function Converter() {
  ...
  const [job, setJob] = useState({ status: "idle", progress: 0, message: "" });
  ...
  const inspectFile = useCallback(async (nextFile) => {
            ...
            <div className="flex flex-wrap items-center gap-4">
              {busy ? (
                <>
                  ...
                  <Button variant="outline" onClick={() => jobRef.current.cancel()}>
                    {t("cancel")}
                  </Button>
                </>
              ) : (
      ...
      {job.status === "done" && job.result && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Button onClick={() => downloadBlob(buildZip(job.result, baseName), `${baseName}-converted.zip`)}>
 ...
 */
function translateWarning(t, warning) {
  try {
